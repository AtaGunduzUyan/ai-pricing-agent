...
#        self.beta_entry = ttk.Entry(self.root)
        ...
        # Set default values
        self.cost1_entry.insert(0, '25')
        self.cost2_entry.insert(0, '25')
 #       self.a_entry.insert(0, '14')
 #       self.d_entry.insert(0, '0.00333333333333')
        self.p1_entry.insert(0, '25')
        self.p2_entry.insert(0, '25')
...
#        self.beta_entry.grid(row=3, column=1)
        ...
        self.p2_entry.grid(row=1, column=2)

        self.load_data_label.grid(row=2, column=0, sticky='w')
        self.load_data_entry.grid(row=2, column=1)

        self.load_strategy_label.grid(row=3, column=0, sticky='w')
        self.load_strategy_entry.grid(row=3, column=1)

        self.has_conversation_label.grid(row=4, column=0, sticky='w')
        self.has_conversation_entry.grid(row=4, column=1)
...
#        else:
#            load_data = ''
        load_data = load_data_key.strip()  # sadece klasör adı, boş olabilir
        ...
        self.data = para_cost, initial_price, load_data, strategy, has_conversation
