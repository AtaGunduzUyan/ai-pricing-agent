...
import src.firm_pricing_competition.customer_environment as env


plt.ion()

# Model Setup
model_ver = "gemini-2.5-pro" # LLM here, e.g., "gpt-3.5-turbo"
...
class Market:
    def __init__(self, firms, rounds = 1000, n_communications = 3, ages=None, incomes=None):
        self.firms = firms
        self.rounds = rounds
        self.n_communications = n_communications
        self.ages = ages
        self.incomes = incomes
        if ages is None or incomes is None:
            raise ValueError("Market requires ages and incomes (customer population).")

    
    def firm_name(self, ids):
        return Data.name_dict.get(ids)

    def simulate(self, initial_price = [0, 0], breakpoint_rounds = 0, output_path = "", current_round = 0):
        logs_conversation = []
        log_price_data = []
        log_price_data_plot = []
        log_strategy = []
        ...
        for firm in self.firms:
            log_settings += f"Firm {firm.id}: cost = {firm.cost}, temp = {firm.temperature}\n"
        ...
        for round in range(current_round, self.rounds):
            ...
            for firm in self.firms:
                ...
                if program_run_dict.get("Conversation") == False:
                    if firm.id == 1:
                        context_game_description = prompts["game_description"].format(firm_name = firm.firm_name, firm_name_2 = self.firm_name(firm.id % len(self.firms) + 1), firm_cost = firm.cost, persona = persona_prompts["firm_persona_{}".format(firm_persona_1)])
                    if firm.id == 2:
                        context_game_description = prompts["game_description"].format(firm_name = firm.firm_name, firm_name_2 = self.firm_name(firm.id % len(self.firms) + 1), firm_cost = firm.cost, persona = persona_prompts["firm_persona_{}".format(firm_persona_2)])
                else:
                    if firm.id == 1:
                        context_game_description = prompts["game_description_conversation"].format(firm_name = firm.firm_name, firm_name_2 = self.firm_name(firm.id % len(self.firms) + 1), firm_cost = firm.cost, persona = persona_prompts["firm_persona_{}".format(firm_persona_1)])
                    if firm.id == 2:
                        context_game_description = prompts["game_description_conversation"].format(firm_name = firm.firm_name, firm_name_2 = self.firm_name(firm.id % len(self.firms) + 1), firm_cost = firm.cost, persona = persona_prompts["firm_persona_{}".format(firm_persona_2)])
                ...
                for i in range(max(0, round - prev_n_rounds), round):
                    context_past_decisions += f"Round #{i + 1}: [{firm.price_history[i]}, {round_function(firm.demand_history[i])}, {round_function(firm.profit_history[i])}]\n"
                
                context_prev_consideration = ""
                if round != 0:
                    if program_run_dict.get("Strategy") and round >= prev_n_rounds and round % prev_n_rounds == 0:
...
#                                v4=round_function(float(sum(firm.rival_price_history[round_lb - 1 : round_rb]))/len(firm.rival_price_history[round_lb - 1 : round_rb])))
            ...
            if program_run_dict.get("Conversation"):
                ...
                for _ in range(self.n_communications):
                    for firm in self.firms:
                        ...
                        if program_run_dict.get("Strategy") and len(firm.strategy) > 0:
            ...
            for firm in self.firms:
                context = ""
                print(f"Firm {firm.firm_name}: Profit {firm.max_profit} with price {firm.max_price}")
                ...
                # Price instructions
                context_price_instructions = prompts["Phase_2_Description_1"]
                
                # Agent make decisions
                # Agent make decisions
                if program_run_dict.get("Conversation") == False:
                    if round < prev_n_rounds:
                        # No strategy block yet (insufficient history)
                        context = (
                            firm.context["context_game_description"]
                            + firm.context["context_prev_consideration"]
                            + firm.context["context_phase_1"]
                            + context_price_instructions
                        )
                    else:
                        # Strategy block is optional and must not break context construction
                        context_strategy = ""
                        if program_run_dict.get("Strategy") and len(firm.strategy) > 0:
                            context_strategy = prompts["Phase_2_Strategy"] + firm.strategy[-1]
            ...
            # 9.1) A-bilgileri environment: tek seferde satışlar ve no-buy üret
            sales_by_firm, no_buy = env.simulate_tick_sales(prices, self.ages, self.incomes)
            ...
            for i, firm in enumerate(self.firms):
                ...
                # 9.2) Bu round'da firmaya düşen satış (integer)
                firm.demand = int(sales_by_firm[i])

                # 9.3) Profit: (p - c) * quantity  (quantity integer)
                firm.profit = int((firm.price - firm.cost) * firm.demand)

                # Max-profit tracker (mevcut mantığı koru)
                if firm.profit > firm.max_profit:
                    firm.max_profit = firm.profit
                    firm.max_price = firm.price
#                    firm.max_rival_price = rival_price
...
#                firm.rival_price_history.append(float(rival_price))
                ...
                # Logs
                logs_round.append(
                    log_format["Phase_3_Log_Format"].format(
                        firm_name=self.firm_name(firm.id),
...
#            log_price_data_dict = {'Round': round + 1}
            log_price_data_dict = {'Round': round + 1, 'NoBuy': no_buy}
            ...
            data_plot.plot_decisions(self.firms)

            # Output data
            df_conversation = pd.DataFrame(logs_conversation, columns=['Round', 'Content'])
            df_decision = pd.DataFrame(log_price_data, columns=['Round', 'Price 1', 'Quantity 1', 'Profit 1', 'Price 2', 'Quantity 2', 'Profit 2', 'NoBuy'])
            ...
            if (round + 1) % (prev_n_rounds * 2) == 0:
                data_plot.data_visulization(df_conversation, df_decision_plot, output_path)
        
        plt.close()
        return logs_conversation, log_price_data, log_price_data_plot, log_strategy, log_settings

def run_simulation(para_cost, initial_price, load_data='', strategy = True, has_conversation = False):
    # Agents Setup    
    firm1 = GPT.Firm(id=1, cost=para_cost[0], temperature=0.7, api_key=my_apikey1, model=model_ver, max_tokens = output_max_tokens)
    firm2 = GPT.Firm(id=2, cost=para_cost[1], temperature=0.7, api_key=my_apikey2, model=model_ver, max_tokens = output_max_tokens)
    firms = [firm1, firm2]
    ...
    program_run_dict["Strategy"] = strategy
    if program_run_dict.get("Conversation"):
    ...
    # Simluation
    if load_data == '':
        ages, incomes = env.generate_population()
        env.save_population(os.path.join(output_path, "customer_population.npz"), ages, incomes)

        market = Market(firms, rounds, n_communications, ages=ages, incomes=incomes)
        logs_conversation, logs_decision, log_decision_plot, log_strategy, log_settings = market.simulate(
            initial_price, breakpoint_rounds, output_path
)

    else:
        program_run_dict['Set_Initial_Prices'] = False

        ages, incomes = env.load_population("output/pricing_competition/" + load_data + "/customer_population.npz")
...
#        firm2.rival_price_history = list(row1['Price'].values)
        ...
        logs_conversation, logs_decision, log_decision_plot, log_strategy, log_settings = market.simulate(initial_price, breakpoint_rounds, output_path, current_round)
    
        env.save_population(os.path.join(output_path, "customer_population.npz"), ages, incomes)

    # Log Output
    df_conversation = pd.DataFrame(logs_conversation, columns=['Round', 'Content'])
    df_decision = pd.DataFrame(logs_decision, columns=['Round', 'Price 1', 'Quantity 1', 'Profit 1', 'Price 2', 'Quantity 2', 'Profit 2', 'NoBuy'])
